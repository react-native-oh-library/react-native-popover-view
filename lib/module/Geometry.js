...
export function computeGeometry(options) {
  ...
  const fromRect = options.fromRect ? Rect.clone(options.fromRect) : null;
  if (fromRect && options.fromRect instanceof Rect) {
    ...
    if (Array.isArray(placement)) {
      ...
      const [bestProvidedPlacement] = placement.filter(p => p === Placement.AUTO || p === Placement.FLOATING || bestPlacements.includes(p));
    ...
    if (newGeom && (newGeom.viewLargerThanDisplayArea.width || newGeom.viewLargerThanDisplayArea.height)) {
      ...
      if (fromRect.width > requestedContentSize.width && fromRectHeightVisible > requestedContentSize.height) {
        ...
        const forcedContentSize = new Size(Math.min(fromRect.width - 20, displayArea.width), Math.min(fromRect.height - 20, displayArea.height));
       ...
       */
      placement === Placement.AUTO && (newGeom.viewLargerThanDisplayArea.width && [Placement.RIGHT, Placement.LEFT].includes(newGeom.placement) || newGeom.viewLargerThanDisplayArea.height && [Placement.TOP, Placement.BOTTOM].includes(newGeom.placement))) {
        newGeom = null;
      }
    }
  }
...
function computeTopGeometry({
  ...
  const forcedContentSize = new Size(displayArea.width, fromRect.y - arrowSize.height - displayArea.y);
  ...
  const popoverOrigin = new Point(Math.min(maxX, Math.max(minX, preferredX)), Math.min(maxY, Math.max(minY, preferredY)));
...
function computeBottomGeometry({
  ...
  const forcedContentSize = new Size(displayArea.width, displayArea.y + displayArea.height - preferedY);
  const viewLargerThanDisplayArea = {
  ...
  const popoverOrigin = new Point(Math.min(maxX, Math.max(minX, preferedX)), Math.min(maxY, Math.max(minY, preferedY)));
...
function computeLeftGeometry({
  ...
  const forcedContentSize = new Size(fromRect.x - displayArea.x - arrowSize.width, displayArea.height);
  const viewLargerThanDisplayArea = {
  ...
  const popoverOrigin = new Point(Math.min(Math.max(minX, preferedX), maxX), Math.min(Math.max(minY, preferedY), maxY));
...
function computeRightGeometry({
  ...
  const forcedContentSize = new Size(horizontalSpace, displayArea.height);
  const viewLargerThanDisplayArea = {
    height: requestedContentSize.height >= displayArea.height + 1,
...
function computeAutoGeometry(options) {
  ...
  if (previousPlacement === Placement.LEFT || previousPlacement === Placement.RIGHT) {
    const geom = previousPlacement === Placement.LEFT ? computeLeftGeometry(options) : computeRightGeometry(options);
  ...
  if (previousPlacement === Placement.TOP || previousPlacement === Placement.BOTTOM) {
    const geom = previousPlacement === Placement.TOP ? computeTopGeometry(options) : computeBottomGeometry(options);
